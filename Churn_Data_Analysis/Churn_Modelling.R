#implementing libraries and installation  packages
library(tidyverse)
library(skimr)
library(inspectdf)
library(caret)
...
# Exclude unimportant variables by IV(>0.02)------------
...
df %>% filter(Geography=='France') %>% count()

df %>% woebin('Exited')->bins
...
pred <- model %>% h2o.predict(newdata = test_h2o) %>% 
  as.data.frame() %>% select(p1,predict) %>% view()
